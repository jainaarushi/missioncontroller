...
import { getTask, getAgent, updateTask as updateMockTask, setMockSteps, markTaskRunning, markTaskDone } from "@/lib/mock-data";
...
export async function POST(
  ...
  // Load Composio tools if available
    ...
    for (let blockIdx = 0; blockIdx < agentBlocks.length; blockIdx++) {
      ...
      for (let i = 0; i < block.pipeline.length; i++) {
        ...
        if (step.isCore) {
          ...
          const stepTools = { ...builtInTools, ...composioTools };
          ...
          if (Object.keys(composioTools).length > 0) {
            const composioToolNames = Object.keys(composioTools).join(", ");
            coreSystem += `\n\nYou also have access to external Composio tools: ${composioToolNames}. Use them when relevant to the task.`;
          }
