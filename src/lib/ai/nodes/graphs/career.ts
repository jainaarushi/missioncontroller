...
export const CAREER_GRAPHS: Record<string, PipelineGraph> = {
  ...
  "auto-applier": {
    nodes: [
      ...
      { id: "search", type: "search", label: "Research Company", description: "Researching the target company", icon: "🔍", color: "#3b82f6", inputs: ["input"], config: { type: "search", queries: (ctx) => { const q = ctx.latestText.slice(0, 60).trim(); return [`${q} company culture values`, `${q} interview process hiring`]; }, maxResults: 5 } },
  ...
  "interview-coach": {
    nodes: [
      ...
      { id: "search_company", type: "search", label: "Research Company", description: "Deep-diving into the company", icon: "🔍", color: "#3b82f6", inputs: ["input"], config: { type: "search", queries: (ctx) => { const q = ctx.latestText.slice(0, 60).trim(); return [`${q} interview questions glassdoor`, `${q} company culture values mission`, `${q} recent news funding`]; }, maxResults: 8 } },
